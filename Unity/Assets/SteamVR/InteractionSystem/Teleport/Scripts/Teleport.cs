	...
	public class Teleport : MonoBehaviour
    {
		...
		private void UpdatePointer()
		{
			...
			if ( hitTeleportMarker != null ) //Hit a teleport marker
			{
				...
				{
					if (hitTeleportMarker.isPointer == true) {
						Debug.Log("this should be black");
						teleportArc.SetColor( isPointerColor );
#if (UNITY_5_4)
						pointerLineRenderer.SetColors( isPointerColor, isPointerColor );
	#else
						pointerLineRenderer.startColor = isPointerColor;
						pointerLineRenderer.endColor = isPointerColor;
	#endif
